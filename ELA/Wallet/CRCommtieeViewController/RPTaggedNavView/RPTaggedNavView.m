...
#define DefaultSliderW (([[UIScreen mainScreen] bounds].size.width) / 3)

#define WTWeakSelf __weak __typeof(self) weakSelf = self
...
- (void)createUI
{
    ...
    for (int i = 0 ; i < _dataArr.count ; i++) {
        ...
        button.titleLabel.font = PingFangRegular(self.tagTextFont_normal);
        [button addTarget:self action:@selector(tagBtnClick:) forControlEvents:UIControlEventTouchUpInside];
        //默认第一个选中
        if (i == 0) {
            button.selected = YES;
            button.titleLabel.font = PingFangRegular(self.tagTextFont_selected);
        }
        
        [self.buttonsArray addObject:button];
        [self addSubview:button];
    }
    UIView *lineView = [[UIView alloc] initWithFrame:CGRectMake(0, (self.sliderH - 0.5) , self.sliderW * 3, 0.5)];
   // [lineView setBackgroundColor:[ELAUtils colorWithHex:0xf1f1f1 alpha:1]];
    lineView.backgroundColor = [UIColor clearColor];
    [self addSubview:lineView];
    
    self.sliderView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.sliderW, self.sliderH)];
...
- (void)tagBtnClick:(UIButton *)btn
{
    ...
    for (UIButton *subButton in self.buttonsArray) {
        ...
        subButton.titleLabel.font = PingFangRegular(self.tagTextFont_normal);
    }
    btn.selected = YES;
    btn.titleLabel.font = PingFangRegular(self.tagTextFont_selected);
    ...
    for (UIButton *subButton in self.buttonsArray) {
        if (s_btnTagIndex != subButton.tag) {
            ...
            [UIView animateWithDuration:0.2 animations:^{
                ...
                subButton.titleLabel.font = PingFangRegular(self.tagTextFont_selected);
            }];
        }
    }
}
    ...
    if (_tagTextFont_normal != tagTextFont_normal) {
        for (UIButton *subButton in self.buttonsArray){
            if (subButton.selected == NO) {
                subButton.titleLabel.font = PingFangRegular(tagTextFont_normal) ;
            }
        }
    ...
    if (_tagTextFont_selected != tagTextFont_selected) {
        for (UIButton *subButton in self.buttonsArray){
            if (subButton.selected == YES) {
                subButton.titleLabel.font = PingFangRegular(tagTextFont_selected);
            }
        }
