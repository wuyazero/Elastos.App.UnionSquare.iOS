...
- (instancetype)safeInitWithObjects:(const id _Nonnull __unsafe_unretained *)objects count:(NSUInteger)cnt {
    ...
    for (NSUInteger i = 0; i < cnt; i++) {
        if ([objects[i] isKindOfClass:[NSArray class]]) {
            ...
            hasNilObject = YES;
            //WYLog(@"%s object at index %lu is nil, it will be filtered", __FUNCTION__, i);
            //#if DEBUG
...
- (void)safeAddObject:(id)obj {
    if (obj == nil) {
        //WYLog(@"%s can add nil object into NSMutableArray", __FUNCTION__);
    } else {
...
- (void)safeRemoveObject:(id)obj {
    if (obj == nil) {
        //WYLog(@"%s call -removeObject:, but argument obj is nil", __FUNCTION__);
        return;
    }
...
- (void)safeInsertObject:(id)anObject atIndex:(NSUInteger)index {
    ...
    if (anObject == nil) {
        //WYLog(@"%s can't insert nil into NSMutableArray", __FUNCTION__);
        
    } else if (index > self.count) {
        //WYLog(@"%s index is invalid", __FUNCTION__);
...
- (id)safeObjectAtIndex:(NSUInteger)index {
    if (self.count == 0) {
        //WYLog(@"%s can't get any object from an empty array", __FUNCTION__);
        return nil;
        
    } if (index >= self.count) {
        //WYLog(@"%s index out of bounds in array", __FUNCTION__);
        return nil;
...
- (void)safeRemoveObjectAtIndex:(NSUInteger)index {
    if (self.count <= 0) {
        ...
        //WYLog(@"%s index out of bound", __FUNCTION__);
        return;
